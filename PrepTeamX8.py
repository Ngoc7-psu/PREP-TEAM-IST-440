Strawberry Lemonade Smoothie#DEVELOPMENT PHASE
...
def Getmotherbrew_tbl():
    ...
    url = 'https://emplkasperpsu1.service-now.com/api/now/table/x_snc_beer_brewing_mother_brewv2?sysparm_query=%5EORDERBYDESCsys_created_on%5Eactive%3Dtrue&sysparm_fields=number%2Cbeer_name%2Cbeer_type%2Corder_id%2Ccustomer_id%2Cgrain_type_1%2Cgrain_type_2%2Cgrain_type_3%2Cgrain_weight%2Cpayment_id%2Ckeg_quantity%2Ckeg_volume%2Ckeg_price%2Cgrain_price%2Cmalt_price%2Chop_price%2Cyeast_price%2Csys_id&sysparm_limit=10'
    ...
    # Decode the JSON response into a dictionary and use the data
    global number, beername, beertype, Grainprice, Grainweight, GrainType1, GrainType2, GrainType3, customer_id, payment_id, order_id, keg_quantity, keg_volume, keg_price, malt_price, hops_price, yeast_price  
    ...
    hops_price = response.json()['result'][0]['hop_price']
    ...
    return number, beername, beertype, Grainprice, Grainweight, GrainType1, GrainType2, GrainType3, customer_id, payment_id, order_id, keg_quantity, keg_volume, keg_price, malt_price, hops_price, yeast_price
Getmotherbrew_tbl()
...
def Create_Price(test=4):
    global Grainprice, malt_price, hops_price, yeast_price, keg_quantity, keg_price, keg_volume
    #total_price = (keg_quantity*keg_cost)
#     print('===')
#     print(type(Grainprice))
#     print(type(malt_price))
#     print(type(hops_price))
...
#     print('+++')
    ...
    hops_price = float(hops_price)
    yeast_price = float(yeast_price)
    keg_quantity = int(keg_quantity)
    keg_price = float(keg_price)
    total_price = (Grainprice + malt_price + hops_price + yeast_price)* keg_quantity * int(keg_volume) * keg_price
